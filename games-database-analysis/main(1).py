...
dr_main = pd.read_csv("game-ratings-by-top-10-genres.csv")
...
df_hype = df_hype.rename(columns = {"hype_count": "hype_follows"})
...
plt.xticks(df_follow.index, df_follows["genre_name"])

plt.legend(loc = "upper left")

plt.show()

dr_main = pd.read_csv("game-ratings-by-top-10-platform.csv")
...
plt.xticks(df_follow.index, df_follows["platform_name"])
